...
%      'edge_labels' -  Cell array (parallel to adj) with edge labels
...
for i = 1:2:nargin-1                  % get optional args
    switch varargin{i}
        ...
        case 'edge_labels', edge_labels = varargin{i+1};
...
for node = 1:N                          %  Draw  edges 
  ...
  for node2 = edges
    ...
    if ~isempty(edge_labels{node, node2})
        text((x(node)+x(node2))/2, (y(node)+y(node2))/2, edge_labels{node, node2}, 'BackgroundColor', [.9 .9 .9])
    end
  end;
end;
